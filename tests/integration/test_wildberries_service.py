...
def test_get_correct_product_info(app_url):
    ...
    assert isinstance(body['basic_kop'], int), \
        "Product have correct basic price (kop)"
    assert isinstance(body['product_kop'], int), \
        "Product have correct product price (kop)"
    assert isinstance(body["basic_rub"], float), \
        "Product have correct basic price (rub)"
    assert isinstance(body["product_rub"], float), \
